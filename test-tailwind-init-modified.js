...
async function testTailwindInit() {
  try {
    ...
    logger.info(`Number of active handles: ${activeHandles.length}`);
    activeHandles.forEach((handle, index) => {
      logger.info(`Handle ${index}: ${handle.constructor.name}`);
    });
    
    // Log active requests that might be keeping the process alive
    const activeRequests = process._getActiveRequests();
    logger.info(`Number of active requests: ${activeRequests.length}`);
    activeRequests.forEach((request, index) => {
      logger.info(`Request ${index}: ${request.constructor.name}`);
    });
